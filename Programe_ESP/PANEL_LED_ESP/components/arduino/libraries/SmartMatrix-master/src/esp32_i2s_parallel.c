...
static void IRAM_ATTR i2s_isr(void *arg) {
  REG_WRITE(I2S_INT_CLR_REG(1),
            (REG_READ(I2S_INT_RAW_REG(1)) & 0xffffffc0) | 0x3f);

  // at this point, the previously active buffer is free, go ahead and write to
  // it
...
// size must be less than DMA_MAX - need to handle breaking long transfer into
// two descriptors before call
...
void i2s_parallel_setup_without_malloc(i2s_dev_t *dev,
  ...
  // Enable "One datum will be written twice in LCD mode" - for some reason, if
  // we don't do this in 8-bit mode, data is updated on half-clocks not clocks
  if (cfg->bits == I2S_PARALLEL_BITS_8)
    dev->conf2.lcd_tx_wrx2_en = 1;
  ...
  dev->sample_rate_conf.rx_bits_mod = cfg->bits;
  dev->sample_rate_conf.tx_bits_mod = cfg->bits;
  dev->sample_rate_conf.rx_bck_div_num =
  ...
  if (cfg->bits == I2S_PARALLEL_BITS_8)
    ...
    dev->sample_rate_conf.tx_bck_div_num =
        1; // datasheet says this must be 2 or greater (but 1 seems to work)
  ...
  // setup I2S Interrupt
  SET_PERI_REG_BITS(I2S_INT_ENA_REG(1), I2S_OUT_EOF_INT_ENA_V, 1,
                    I2S_OUT_EOF_INT_ENA_S);
  // allocate a level 1 intterupt: lowest priority, as ISR isn't urgent and may
  // take a long time to complete
  esp_intr_alloc(ETS_I2S1_INTR_SOURCE,
                 (int)(ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_LEVEL1), i2s_isr,
                 NULL, NULL);
...
void i2s_parallel_flip_to_buffer(i2s_dev_t *dev, int bufid) {
  ...
  // setup linked list to refresh from new buffer (continuously) when the end of
  // the current list has been reached
  ...
  // we're still refreshing the previously buffer, so it shouldn't be written to
  // yet
